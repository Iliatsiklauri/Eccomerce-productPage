...
const Button = ({ text, display }: textType) => {
  ...
  return (
    <div
      className="h-14 rounded-lg mt-4 w-full flex items-center justify-center bg-[#FF7E1B] cursor-pointer hover:opacity-45"
      onClick={() => {
        if (display) {
          setPush(true);
          console.log(push);
    ...
    >
      <div className="flex items-center justify-center gap-4">
        ...
        <h3 className="text-white font-bold text-base">{text}</h3>
      </div>
    </div>
  );
};
